...
namespace UltEvents.Editor {
  ...
  internal sealed class PersistentArgumentDrawer : PropertyDrawer {
    ...
    private static readonly GUIContent _LinkToggleContent = new GUIContent("∞", "Link to Parameter or Returned Value");
    ...
    private static void DoBoolGUI(Rect area, SerializedProperty argumentProperty, GUIContent label) {
      ...
      if (EditorGUI.EndChangeCheck()) i.intValue = value ? 1 : 0;
      EditorGUI.EndProperty();
    }
    ...
    private static void DoStringGUI(Rect area, SerializedProperty argumentProperty, GUIContent label) {
      ...
      if (EditorGUI.EndChangeCheck()) s.stringValue = value;
      EditorGUI.EndProperty();
    }
    ...
    private static void DoIntGUI(Rect area, SerializedProperty argumentProperty, GUIContent label) {
      ...
      if (EditorGUI.EndChangeCheck()) i.intValue = value;
      EditorGUI.EndProperty();
    }
    /************************************************************************************************************************/
    ...
    private static void DoFloatGUI(Rect area, SerializedProperty argumentProperty, GUIContent label) {
      ...
      if (EditorGUI.EndChangeCheck()) x.floatValue = value;
      EditorGUI.EndProperty();
    }
    ...
    private static void DoObjectGUI(Rect area, SerializedProperty argumentProperty, GUIContent label) {
      ...
      var type = argumentProperty.GetValue<PersistentArgument>().SystemType ?? typeof(Object);

      var value = EditorGUI.ObjectField(area, label, o.objectReferenceValue, type, true);

      if (EditorGUI.EndChangeCheck()) o.objectReferenceValue = value;
      EditorGUI.EndProperty();
    }
    ...
    private static void ShowLinkMenu(Rect area, SerializedProperty argumentProperty, Type systemType, int linkIndex,
      ...
      for (var i = 0; i < DrawerState.Current.Event.ParameterTypes.Length; i++) {
      ...
      // Returned Values.
      for (var i = 0; i < DrawerState.Current.PreviousCallCount; i++) {
        ...
        argumentProperty.ModifyValues<PersistentArgument>(argument => {
          if (wasLink) {
            ...
            switch (argumentType) {
              ...
              default:
                throw new InvalidOperationException("Invalid " + Names.PersistentArgument.Full.Type + ": " +
                                                    argumentType);
            }
          }
