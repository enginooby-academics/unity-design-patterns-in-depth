...
namespace UltEvents.Editor {
    ...
    public static SerializedProperty GetMethodNameProperty(SerializedProperty callProperty) =>
      callProperty.FindPropertyRelative(Names.PersistentCall.MethodName);
    ...
    public static PersistentCall GetCall(SerializedProperty callProperty) => callProperty.GetValue<PersistentCall>();
    /************************************************************************************************************************/
    ...
    public void CacheLinkedArguments() {
      ...
      for (var i = 0; i < Event._PersistentCalls.Count; i++) {
    ...
    public void UpdateLinkedArguments() {
      ...
      for (var i = 0; i < Event._PersistentCalls.Count; i++) {
        ...
        for (var j = 0; j < call._PersistentArguments.Length; j++) {
    ...
    public int IndexOfMethod(MethodBase method) {
      for (var i = 0; i < Event._PersistentCalls.Count; i++) {
        var call = Event._PersistentCalls[i];
        if ((call != null ? call.GetMethodSafe() : null) == method) return i;
      }
    ...
    public bool TryGetLinkable(Type type, out int linkIndex, out PersistentArgumentType linkType) {
      if (Event != null) {
        ...
        // Return Values.
        for (var i = 0; i < PreviousCalls.Count; i++) {
