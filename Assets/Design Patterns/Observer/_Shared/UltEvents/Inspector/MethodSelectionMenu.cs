...
namespace UltEvents.Editor {
    ...
    private static Object ValidateTargetsAndGetFirst(Object[] targets) {
      ...
      for (; i < targets.Length; i++) {
        var obj = targets[i];
        if (obj == null || obj.GetType() != targetType) return null;
      }
    ...
    private static T[] GetRelatedObjects<T>(Object[] objects, System.Func<Object, T> getRelatedObject) {
      ...
      for (var i = 0; i < relatedObjects.Length; i++) relatedObjects[i] = getRelatedObject(objects[i]);
    ...
    private static void AddSelectPropertyItem(string prefix, Object[] targets, Type currentType, PropertyInfo property,
      MethodInfo getter) {
    ...
    public static bool IsSupported(Type type) {
      if (PersistentCall.IsSupportedNative(type)) return true;
