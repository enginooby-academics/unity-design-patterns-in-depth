...
namespace UltEvents.Editor {
  ...
  public static class ObjectPicker {
    ...
    /// <summary>Draws a field which lets you pick an object from a list and returns the selected object.</summary>
    public static T Draw<T>(Rect area, T selected, Func<List<T>> getOptions, int suggestions,
      Func<T, GUIContent> getLabel, Func<T> getDragAndDrop) => Draw(area, selected, getOptions, suggestions, getLabel,
      getDragAndDrop, InternalGUI.TypeButtonStyle);

    /************************************************************************************************************************/

    /// <summary>Draws a field (using GUILayout) which lets you pick an object from a list and returns the selected object.</summary>
    public static T DrawLayout<T>(T selected, Func<List<T>> getOptions, int suggestions, Func<T, GUIContent> getLabel,
    ...
    public static T DrawLayout<T>(T selected, Func<List<T>> getOptions, int suggestions, Func<T, GUIContent> getLabel,
      ...
      DrawLayout(selected, getOptions, suggestions, getLabel, getDragAndDrop, InternalGUI.TypeButtonStyle,
        layoutOptions);
    ...
    public static T DrawEditorLayout<T>(GUIContent label, T selected, Func<List<T>> getOptions, int suggestions,
      ...
      DrawEditorLayout(label, selected, getOptions, suggestions, getLabel, getDragAndDrop, InternalGUI.TypeButtonStyle,
        options);
    ...
    public static Type DrawTypeField(Rect area, Type selected, Func<List<Type>> getOptions, int suggestions,
      ...
      return Draw(area, selected, getOptions, suggestions,
        type => new GUIContent(type != null ? type.GetNameCS() : "null"),
    ...
    public static Type DrawAssetTypeField(Rect area, Type selected, Func<List<Type>> getOptions, int suggestions,
      GUIStyle style) {
      return Draw(area, selected, getOptions, suggestions,
        type => new GUIContent(type != null ? type.GetNameCS() : "null", AssetPreview.GetMiniTypeThumbnail(type)),
        () => DragAndDrop.objectReferences[0].GetType(),
        style);
    }
    ...
    /// </summary>
    public static string DrawTypeField(Rect area, string selectedTypeName, Func<List<Type>> getOptions, int suggestions,
      GUIStyle style) {
      var selected = Type.GetType(selectedTypeName);

      selected = Draw(area, selected, getOptions, suggestions,
        type => new GUIContent(type != null ? type.GetNameCS() : "No Type Selected"),
        () => DragAndDrop.objectReferences[0].GetType(),
        style);
    ...
    public static void RemoveDuplicateSuggestions<T>(List<T> options, int suggestions) where T : class {
      for (var i = options.Count - 1; i >= suggestions; i--) {
        var obj = options[i];
        for (var j = 0; j < suggestions; j++)
          if (obj == options[j]) {
            options.RemoveAt(j);
            break;
          }
  ...
  internal sealed class ObjectPickerWindow : EditorWindow {
    ...
    public static void Show<T>(int fieldID, T selected, List<T> objects, int suggestions,
      ...
      if (selected != null) {
        ...
        for (var i = 0; i < window._Objects.Count; i++)
    ...
    private void DrawSearchBar(ref Rect area) {
      ...
      GUI.BeginGroup(area, EditorStyles.toolbar);
      {
        ...
        if (HasSearchText) {
          if (GUI.Button(area, "", InternalGUI.SearchBarCancelStyle)) _SearchText = "";
        }
        else {
    ...
    private static bool IsVisibleInSearch(string search, string text) =>
      CultureInfo.CurrentCulture.CompareInfo.IndexOf(text, search, CompareOptions.IgnoreCase) >= 0;
    ...
    private void DrawOption(Rect area, List<GUIContent> labels, IList objects, int index) {
      ...
      if (isOn != wasOn) {
        if (wasOn)
          PickAndClose();
        else if (isOn) _SelectedObject = obj;
      }
    }
