...
namespace UltEvents.Editor {
  ...
  internal sealed class UltEventDrawer : PropertyDrawer {
    ...
    static UltEventDrawer() => HeaderBackground.fixedHeight -= 1;
    ...
    public void DoFooterGUI(Rect area) {
      ...
      if (DrawerState.Current.EventProperty.serializedObject.targetObjects.Length > 1) {
        ...
        if (dynamicListenerCount > 0) {
          var isExpanded =
            EditorGUI.Foldout(area, _CurrentCallList.serializedProperty.isExpanded, GUIContent.none, true);
      ...
      if (GUI.Button(area, ReorderableList.defaultBehaviours.iconToolbarPlus,
            ReorderableList.defaultBehaviours.preButton)) AddNewCall(_CurrentCallList);
      ...
      using (new EditorGUI.DisabledScope(_CurrentCallList.index < 0 || _CurrentCallList.index >= _CurrentCallCount)) {
        if (GUI.Button(area, ReorderableList.defaultBehaviours.iconToolbarMinus,
              ReorderableList.defaultBehaviours.preButton)) DelayedRemoveCall(_CurrentCallList.index);
      }
    }
    ...
    private void CheckDragDrop(Rect area) {
      ...
      switch (Event.current.type) {
        ...
        case EventType.DragPerform:
          foreach (var drop in DragAndDrop.objectReferences) AddNewCall(_CurrentCallList, drop);
          DrawerState.Current.EventProperty.isExpanded = true;
