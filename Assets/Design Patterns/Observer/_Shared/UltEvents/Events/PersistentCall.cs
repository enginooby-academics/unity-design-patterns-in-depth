...
#endif
  {
    ...
    public void SetMethod(MethodBase method, Object target) {
      ...
      if (method != null) {
        ...
        for (var i = 0; i < _PersistentArguments.Length; i++) {
    ...
    public object Invoke() {
      ...
      if (_PersistentArguments != null && _PersistentArguments.Length > 0) {
        ...
        for (var i = 0; i < parameters.Length; i++) parameters[i] = _PersistentArguments[i].Value;
      }
      else {
        parameters = null;
      }
    ...
    internal void GetMethodDetails(out Type declaringType, out string methodName) {
      ...
      if (_Target == null && !ReferenceEquals(_Target, null))
        _Target = EditorUtility.InstanceIDToObject(_Target.GetInstanceID());
#endif
    ...
    public MethodBase Method {
      get {
        if (_Method == null) {
          ...
          for (var i = 0; i < argumentCount; i++) parameters[i] = _PersistentArguments[i].SystemType;
    ...
    private void ClearCache() {
      ...
      for (var i = 0; i < _PersistentArguments.Length; i++) _PersistentArguments[i].ClearCache();
    }
