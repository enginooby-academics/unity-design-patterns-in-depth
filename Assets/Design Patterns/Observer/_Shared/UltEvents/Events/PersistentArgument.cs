...
namespace UltEvents {
  ...
  public sealed class PersistentArgument {
    ...
    private void AssertType(PersistentArgumentType type) {
      if (_Type != type)
        throw new InvalidOperationException(Names.PersistentArgument.Full.Type + " is " + _Type + " but should be " +
                                            type);
    }
    ...
    ///   (otherwise null).
    /// </summary>
    public static PersistentArgumentType
      GetArgumentType(Type type, out string assemblyQualifiedName, out int linkIndex) {
      linkIndex = 0;
