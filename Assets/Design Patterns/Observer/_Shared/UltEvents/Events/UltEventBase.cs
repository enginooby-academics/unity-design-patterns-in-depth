  ...
  public abstract class UltEventBase : IUltEventBase {
    ...
    protected void InvokePersistentCalls() {
      ...
      try {
        if (_PersistentCalls != null)
          for (var i = 0; i < _PersistentCalls.Count; i++) {
    ...
    public virtual void CopyFrom(UltEventBase target) {
      if (target._PersistentCalls == null) {
        ...
        for (var i = 0; i < target._PersistentCalls.Count; i++) {
    ...
    internal bool HasAnyDynamicCalls() => DynamicCallsBase != null;
    ...
    internal static object GetParameterValue(int index) => LinkedValueCache[_ParameterOffset + index];

    internal static object GetReturnedValue(int index) => LinkedValueCache[_ReturnValueOffset + index];
