...
namespace Prototype {
  public class ProceduralCylinder : ProceduralShape {
    ...
    public ProceduralCylinder(string name, Color color, Vector3 position, float radius = 1f, int segments = 16,
      float height = 1f)
    ...
    protected override void CreateMeshData() {
      var segmentAngle = 360f / _segments;
      float currentAngle = 0;
      var halfHeightUp = Vector3.up * _height / 2;
      ...
      for (var i = 0; i < _segments; i++) {
        AddCylinderPoints(_radius, currentAngle, halfHeightUp, ref strip, ref stripUV, ref stripNormals,
          out var lowerVertex, out var upperVertex);
        lowerRing.Add(lowerVertex);
        upperRing.Add(upperVertex);
        var uv = PointOnCircle2(0.5f, currentAngle) + new Vector2(0.5f, 0.5f);
        upperDiskUV.Add(uv);
        uv.x = 1 - uv.x;
        lowerDiskUV.Add(uv);
    ...
    private void AddCylinderPoints(float radius, float currentAngle, Vector3 halfHeightUp,
      ...
      var normal = PointOnCircle3(0, 2, 1, currentAngle);
