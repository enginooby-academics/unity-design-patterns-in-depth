...
namespace Prototype.Base {
  public class ProceduralCube : Prototype.ProceduralCube, ICloneable {
    public ProceduralCube(string name, Color color, Vector3 position, float size = 1) : base(name, color, position,
      size) {
    }
    ...
    public object CloneWithColor(Color color) => new ProceduralCube(Name, color, Position, _size);
  }
}