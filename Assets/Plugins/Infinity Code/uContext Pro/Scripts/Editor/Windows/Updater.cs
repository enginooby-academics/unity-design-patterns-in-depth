...
namespace InfinityCode.uContextPro.Windows
{
    ...
    {
        private const string packageID = "ucontext";
        ...
        public static void CheckNewVersionAvailable()
        {
            ...
            client.UploadDataCompleted += delegate (object sender, UploadDataCompletedEventArgs response)
            {
                ...
                {
                    Debug.Log("Automatic check for uContext updates: Bad response.");
                    return;
                }
        ...
        {
            double v1 = GetDoubleVersion(uContextMenu.version);
        ...
        private void GetUpdateList(string updateKey)
        {
            ...
            try
            {
                response = client.UploadString("http://infinity-code.com/products_update/checkupdates.php",
                    "k=" + UnityWebRequest.EscapeURL(updateKey) + "&v=" + uContextMenu.version + "&c=" + (int)channel);
            }
        ...
        private void OnGUI()
        {
            ...
            GUILayout.Label("Current version: " + uContextMenu.version);
        ...
        {
            GetWindow<Updater>(false, "uContext Updater", true);
        }
