...
#pragma warning restore 0414, 0067, 0649
    ...
    private string GetTableGenerationText() {
      ...
      if (QuantumConsoleProcessor.TableIsGenerating) { text += "..."; } else { text += ColorExtensions.ColorText($"\nQ:\\>Quantum Console Processor ready", _theme ? _theme.SuccessColor : Color.white); }
    ...
    private void ProcessCommandHistory() {
      if (InputHelper.GetKeyDown(_keyConfig.NextCommandKey) || InputHelper.GetKeyDown(_keyConfig.PreviousCommandKey)) {
        if (InputHelper.GetKeyDown(_keyConfig.NextCommandKey)) { _selectedPreviousCommandIndex++; } else if (_selectedPreviousCommandIndex > 0) { _selectedPreviousCommandIndex--; }
    ...
    private void ProcessAutocomplete() {
      if ((_keyConfig.SuggestNextCommandKey.IsPressed() || _keyConfig.SuggestPreviousCommandKey.IsPressed()) && !string.IsNullOrWhiteSpace(_currentText)) {
        ...
        if (_suggestedCommands.Count > 0) {
          if (_keyConfig.SuggestPreviousCommandKey.IsPressed()) { _selectedSuggestionCommandIndex--; } else if (_keyConfig.SuggestNextCommandKey.IsPressed()) { _selectedSuggestionCommandIndex++; }
    ...
    private void ProcessPopupDisplay() {
      if (string.IsNullOrWhiteSpace(_currentText)) { ClearPopup(); } else {
        if (_selectedSuggestionCommandIndex < 0) { GetCommandSuggestions(); }
        if (_suggestedCommands.Count == 0) { ClearPopup(); } else {
    ...
    public object InvokeCommand(string command) {
      ...
      if (!string.IsNullOrWhiteSpace(command)) {
        ...
        } catch (System.Reflection.TargetInvocationException e) { logTrace = GetInvocationErrorMessage(e.InnerException); } catch (Exception e) { logTrace = GetErrorMessage(e); }
    ...
    private void HandleAsyncJobCounter() {
      if (_showCurrentJobs) {
        if (_jobCounterRect && _jobCounterText) {
          if (_currentTasks.Count == 0) { _jobCounterRect.gameObject.SetActive(false); } else {
