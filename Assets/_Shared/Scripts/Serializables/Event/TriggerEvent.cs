...
public class TriggerEvent {
  [HorizontalGroup("Group 1")] [PropertyOrder(-10)]
  public bool enable = true;

  [HorizontalGroup("Group 1")] [EnableIf(nameof(enable))] [PropertyOrder(-10)]
  public float delay;
  ...
  [EnableIf(nameof(enable))] [LabelText("SFX")]
  public AudioClip Sfx;

  [EnableIf(nameof(enable))]
  [BoxGroup("After Action")]
#if ODIN_INSPECTOR
  [ValidateInput(nameof(ValidDestroyTarget), "Destroy target is not proper to the current event!",
    InfoMessageType.Warning)]
#endif
  [EnumToggleButtons]
  [LabelText("Destroy")]
  public DestroyTarget destroyAfterInvoked;

  [EnableIf(nameof(enable))] [BoxGroup("After Action")] [EnumToggleButtons] [LabelText("Release To Pool")]
  public ReleaseToPoolTarget releaseToPoolTargetAfterInvoked;

  [EnableIf(nameof(enable))]
  [BoxGroup("After Action")]
  [Tooltip(
    "Changing tag is useful to make the event trigger only once w/o distroying target/collider. Not set if leave field empty.")]
  [LabelText("Set Other Tag")]
  public string otherNewTag;

  [EnableIf(nameof(enable))]
  [BoxGroup("After Action")]
  [Tooltip("Not set if leave field empty.")]
  [LabelText("Set Other Name")]
  public string otherNewName;
