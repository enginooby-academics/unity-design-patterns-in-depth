...
public static class GizmosUtils {
  ...
  /// </summary>
  public static void DrawGizmozRangeFromPos(Vector3 pos, Vector2 range, Axis axisRange, Color? color = null, float thickness = 2f) {
#if UNITY_EDITOR
    Handles.color = (color is null) ? Color.magenta : color.Value;
    Vector3 lowerBound = new Vector3();
    Vector3 higherBound = new Vector3();
  ...
  /// </summary>
  public static void DrawGizmozRangeFromPos(this MonoBehaviour monoBehaviour, Vector2 range, Axis axisRange, Color? color = null, float thickness = 2f) {
    DrawGizmozRangeFromPos(pos: monoBehaviour.transform.position, range: range, axisRange: axisRange, color: color);
  }
}